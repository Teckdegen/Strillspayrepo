...
const Confirmation = () => {
  ...
  const normalizedState = React.useMemo(() => ({
    ...state,
    mobileNumber: state.mobileNumber || state.phone || state.recipient,
  ...
  const { writeContract, error } = useWriteContract({
    mutation: {
      onSuccess: (hash) => {
        ...
        // Prepare success state with 'ready' status for service activation
        const successState = {
          status: 'ready' as const,
          service: state.service,
          provider: state.provider,
          amount: state.amount,
          recipient: normalizedState.mobileNumber,
          mobileNumber: normalizedState.mobileNumber,
          planId: normalizedState.planId,
          networkId: normalizedState.networkId,
          smartCardNumber: normalizedState.smartcard,
          cableId: normalizedState.cableId,
          ...
          plan: state.plan?.name || state.planId,
          network: state.networkId || state.provider,
        };

        console.log('Transaction submitted, navigating to success with ready state:', successState);
  ...
  useEffect(() => {
    const loadExchangeRate = async () => {
      try {
        ...
        toast.error('Failed to load exchange rate. Please try again.');
      } finally {
        setLoading(false);
      }
    };
          ...
          <LitCard className="space-y-6">
            <div className="space-y-4">
              <div className="border-b border-border pb-4">
                ...
                <div className="space-y-3">
                  ...
                  {normalizedState.mobileNumber && (
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Mobile Number:</span>
                      <span className="text-foreground font-medium">{normalizedState.mobileNumber}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex space-x-4">
              ...
              <LitButton 
                ...
                glow
              >
                {processing ? 'Processing...' : loading ? 'Loading...' : 'Confirm Payment'}
              </LitButton>
            </div>
          </LitCard>
        </div>
